...
private:
    ...
    SnubDodecMesh snub;
    PlanetMesh1 pla; //
    Regular_Dodecahedron_Mesh Dod; //정십이면체
    Truncated_Icosahedron_Mesh Ico; //축구공
    ////////////////////////
    Render render;
...
Client::Client(void) {
    ...
    float planet_size[] = { 3.f, 0.5f, 1.f, 0.7f, 0.7f, 2.0f, 3.f, 1.0f, 1.0f};

    planet sun(Ico, RED, 0, 0, planet_size[0]);
    planet mercury(Dod, Mer, 8, 8, planet_size[1]);
    planet venus(snub, Ven, 10, 6, planet_size[2]);
    planet earth(Ico, Earth, 12, 5, planet_size[3]);
    planet mars(snub, Mar, 14, 4, planet_size[4]);
    planet jupiter(Ico, Jup, 24, 4, planet_size[5]);
    planet saturn(snub, Sat, 32, 3, planet_size[6], Vector(-1, 1, 0));
    planet uranos(Dod, Ura, 38, 2, planet_size[7]);
    planet neptune(Dod, Nep, 44, 2.5f, planet_size[8]);
    ...
    for (int i = 0; i < star_num; i++) {
        if (i < 15) {
            ...
            pos = Vector(frand(-100, 100), frand(-20, 20), frand(-100, -150));
        }
        else if (i >= 40 && i < 50) {
            pos = Vector(frand(-100, 100), frand(-20, 20), frand(100, 150));
        }
